...
def _list_values(ws, start_row: int, end_row: int, col: str = "N") -> list[Any]:
    values: list[Any] = []
...
def get_template_metadata() -> dict[str, Any]:
    ...
    defaults = {label: ws[cell].value for label, cell in BASE_INPUT_CELLS.items()}
    defaults["pathology_rows"] = [
        {
            "pathology": ws[f"J{row}"].value or "",
            "grade": ws[f"K{row}"].value if ws[f"K{row}"].value is not None else "",
        }
        for row in PATHOLOGY_ROWS
    ]

    pathology_options = [str(v) for v in _list_values(ws, 5, 9, "N")]
    grade_options = [str(v) for v in _list_values(ws, 12, 14, "N")]
...
def calculate_outputs(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    for label, cell in BASE_INPUT_CELLS.items():
        sheet[cell] = inputs[label]

    pathology_rows = inputs.get("pathology_rows", [])
    for idx, row_num in enumerate(PATHOLOGY_ROWS):
        row_data = pathology_rows[idx] if idx < len(pathology_rows) else {}
        pathology = row_data.get("pathology", "")
        grade = row_data.get("grade", "")
        sheet[f"J{row_num}"] = pathology if pathology != "" else None
        sheet[f"K{row_num}"] = int(grade) if grade not in ("", None) else None
